...
export function UserManager() {
  ...
  const loadUsers = useCallback(async (options?: { silent?: boolean; bustCache?: boolean }) => {
    const cacheKey = `users:q=${filters.q}:status=${filters.status}:page=${page}`;

    if (options?.bustCache) {
      invalidateListCache("users:");
    }

    const cached = readListCache<PaginatedUsers>(cacheKey);
  ...
  const handleSave = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await loadUsers({ bustCache: true, silent: true });
    } catch (error) {
      toast.error(getErrorMessage(error));
    } finally {
      setIsSaving(false);
