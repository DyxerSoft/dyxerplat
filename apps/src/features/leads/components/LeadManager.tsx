...
export function LeadManager() {
  ...
  const loadLeads = useCallback(async (options?: { silent?: boolean; bustCache?: boolean }) => {
    const cacheKey = `leads:q=${filters.q}:status=${filters.status}:page=${page}`;

    if (options?.bustCache) {
      invalidateListCache("leads:");
    }

    const cached = readListCache<PaginatedLeads>(cacheKey);
  ...
  const handleSave = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await loadLeads({ bustCache: true, silent: true });
    } catch (error) {
      toast.error(getErrorMessage(error));
    } finally {
      setIsSaving(false);
