...
import { isProtectedRoleCode, PROTECTED_ROLE_CODES } from "@dyxerplat/shared";
...
async function validateRoles(roleIds: string[]) {
  ...
  if (roles.some((role) => isProtectedRoleCode(role.code))) {
    throw new AppError("No se pueden asignar roles reservados del sistema.", 422, "PROTECTED_ROLE");
  }
